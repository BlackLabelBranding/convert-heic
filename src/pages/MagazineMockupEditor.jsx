...
export default function MagazineMockupPreview() {
  ...
  function renderSinglePage(page, opts = {}) {
    const { mini = false, flushLeft = false, flushRight = false } = opts;

    const shellStyle = mini ? styles.miniPageShell : styles.singlePageShell;
    const innerStyle = mini
      ? styles.miniPageInner
      : {
          ...styles.singlePageInner,
    ...
    if (!page) {
      return (
        <div style={shellStyle}>
          <div style={mini ? styles.miniPageEmpty : styles.singlePageEmpty}>
            No page
          </div>
        </div>
      );
        ...
        >
          {currentItem?.type === "cover" ? (
            <div style={styles.coverStage}>
              {renderSinglePage(currentItem.right)}
            </div>
          ) : currentItem?.type === "single" ? (
            <div style={styles.coverStage}>
              {renderSinglePage(currentItem.left)}
            </div>
          ) : (
            <div style={styles.trueSpreadWrap}>
              ...
              <div style={styles.trueSpread}>
                <div style={styles.trueSpreadPageLeft}>
                  {renderSinglePage(currentItem?.left, { flushRight: true })}
                </div>

                <div style={styles.trueSpreadSeam} />

                <div style={styles.trueSpreadPageRight}>
                  {renderSinglePage(currentItem?.right, { flushLeft: true })}
                </div>
              </div>
            </div>
          )}
            ...
            >
              {item.type === "spread" ? (
                <div style={styles.spreadMiniWrap}>
                  {renderSinglePage(item.left, { mini: true })}
                  {renderSinglePage(item.right, { mini: true })}
                </div>
              ) : (
                renderSinglePage(item.right || item.left, { mini: true })
              )}
